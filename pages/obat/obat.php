    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr>
                ...
                <td>
                    ...
                    <a href='pages/obat/editObat.php?id={$row['id']}'button type='button' class='btn btn-sm btn-warning edit-btn' data-obatid='{$row['id']}'>Edit</button>
...
<script>
    $(document).ready(function() {
        $('btn btn-sm btn-warning edit-btn').on('click', function() {
            var dataId = $(this).data('obatid'); // obatid didapat dari id yang dikirimkan melalui tombol edit
            $('#segmodal').load('pages/obat/editObat.php?id=' + dataId, function() {
                $('#myModal').modal('show');
            });	
        });
    });
